// Public web pages for Cappers & Code: landing page with web sign-in and checkout, /success after
// Stripe Checkout, and the legal pages under /legal/*. No secrets: only the public URL and anon key
// are sent to the browser; checkout itself runs through the JWT-protected stripe-checkout function.
import { createClient } from 'npm:@supabase/supabase-js@2';

const SUPABASE_URL = Deno.env.get('SUPABASE_URL')!;
const ANON_KEY = Deno.env.get('SUPABASE_ANON_KEY')!;
const admin = createClient(SUPABASE_URL, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
const FAVICON = "data:image/svg+xml," + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="12" fill="#050608"/><text x="32" y="44" text-anchor="middle" font-family="Impact,Arial Narrow,sans-serif" font-size="30" font-weight="700" fill="#F5F7F2">CC</text><path d="M10 20 L4 32 L10 44 M54 20 L60 32 L54 44" fill="none" stroke="#B6FF00" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>');
const LOCKUP = '<span class="logo"><span class="mark"><span class="brk">&lt;</span>CC<span class="brk">/&gt;</span></span><small>Cappers <em>&amp;</em> Code</small></span>';
...
function page(title: string, body: string, opts: { script?: string; description?: string } = {}) {
...
// ---------- Landing ----------
function landing(founderEnd: string, discordInvite: string | null) {
  const founderDate = new Date(founderEnd).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  const body = `
...
<p class="mono" style="margin-top:18px">NFL now &middot; CFB and NBA coming</p></div>
...
<div class="panel"><h3>The app</h3><p>iOS and Android app with the live board, results, and record. Web board for everyone else.</p></div>
</div></section>
...
<div class="row"><a class="cta ghost hidden" id="openapp" href="cappers://">Open the app</a>${discordInvite ? `<a class="cta ghost hidden" id="discord" href="${esc(discordInvite)}" rel="noopener">Join the Discord</a>` : ''}<button class="cta ghost hidden" id="portal" type="button">Manage billing</button><button class="cta ghost" id="signout" type="button">Sign out</button></div>
...
// ---------- Success ----------
function success(discordInvite: string | null) {
  const body = `<div class="hero"><div class="kicker">Payment received</div><h1>You're <em>in.</em></h1>
<p class="lead">Your membership activates within seconds. Open the app and pull to refresh, or sign in on the web board. The Discord role follows once you link your account.</p>
<div class="row"><a class="cta" href="cappers://">Open the app</a>${discordInvite ? `<a class="cta ghost" href="${esc(discordInvite)}" rel="noopener">Join the Discord</a>` : '<span class="mono">Discord invite arrives with the members bot.</span>'}<a class="cta ghost" href="./">Back to the site</a></div>
...
const DRAFT = '<div class="draft">Draft v0.1 &middot; pending founder review &middot; September 2026</div>';
function legalPage(title: string, html: string, support: string) {
  return page(`${title} — Cappers & Code`, `<div class="legal"><div class="kicker">Legal</div><h1 style="font-size:clamp(38px,6vw,64px)">${esc(title)}</h1>${DRAFT}${html.replaceAll('{{support}}', esc(support))}</div>`);
}
const TERMS = `
...
Deno.serve(async (req) => {
  ...
  const path = (idx >= 0 ? url.pathname.slice(idx + 5) : url.pathname).replace(/\/+$/, '') || '/';
  const headers = { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=300' };
  if (path === '/health') return new Response(JSON.stringify({ ok: true, ts: new Date().toISOString() }), { headers: { 'Content-Type': 'application/json' } });
  const [support, founderEnd, discordInvite, siteUrl] = await Promise.all([cfg('support_email'), cfg('founder_season_end'), cfg('discord_invite_url'), cfg('site_url')]);
  // The runtime sees /site/..., the public URL is /functions/v1/site/...; site_url in pipeline_config is the public base
  // (it moves to the custom domain later without touching this code).
  const base = (siteUrl ?? `${url.origin}/functions/v1/site`).replace(/\/+$/, '');
  const html = (s: string, status = 200) => new Response(s.replaceAll('href="./', `href="${base}/`), { status, headers });
  const sup = support ?? 'support@cappersandcode.com';
  if (path === '/') return html(landing(founderEnd ?? '2027-06-30T23:59:59Z', discordInvite));
  if (path === '/success') return html(success(discordInvite).replace('<span id="support"></span>', esc(sup)));
  if (path === '/legal/terms') return html(legalPage('Terms of Service', TERMS, sup));
  if (path === '/legal/privacy') return html(legalPage('Privacy Policy', PRIVACY, sup));
  if (path === '/legal/refunds') return html(legalPage('Refund Policy', REFUNDS, sup));
  if (path === '/legal/responsible-gambling') return html(legalPage('Responsible Gambling', RG, sup));
  return html(page('Not found — Cappers & Code', '<div class="hero"><h1>404</h1><p class="lead">That page is not on the board. <a href="./">Back to the site</a>.</p></div>'), 404);
});
