...
4. `node scripts/validate-research.mjs` must pass, and `node scripts/check-rosters.mjs` must report 0 problems (every pick and prop lean matched to its team on ESPN's current roster, spelled ESPN's way).
